...
from detectron2.layers import ShapeSpec, batched_nms, cat, FrozenBatchNorm2d
...
class NNRetinaNet(nn.Module):
	...
	def __init__(self, cfg):
		...
		self.num_keypoint_candidates  = 5
		self.max_kp_boxes_per_image   = 5
		self.nms_threshold_kp         = 0.7
		# Vis parameters
		...
		self.roi_kp_head = build_roi_heads(cfg, self.backbone.output_shape())
	...
	def forward(self, batched_inputs):
		...
		if "instances" in batched_inputs[0]:
			...
			gt_instances = None
		if self.training:
			im_names = [x['file_name'] for x in batched_inputs]
			print('****************************************************************')
			print(im_names)
			print('****************************************************************')
			print(gt_instances)
			print(gt_instances[0].gt_keypoints.tensor)
			print('****************************************************************')
		features = self.backbone(images.tensor)
		kp_map = features['kp_map']
		...
		if self.training:
			...
			if len(kp_img_ids)>0:
				...
				losses = self.roi_kp_head(images, keypoint_feature_map, pred_kp_bboxes, keypoint_gt_instances)
	...
	def transform_delta_to_bbox(self,box_cls,box_delta,anchors,gt_classes,image_sizes,threshold=0.5):
		...
		for img_idx, anchors_per_image in enumerate(anchors):
			...
			results_per_image = self.transform_deltas_to_bbox_per_image(
				box_cls_per_image, box_reg_per_image, anchors_per_image, gt_classes[img_idx], tuple(image_size),threshold
			)
			if results_per_image == -1:
				continue
	...
	def inference_mod(self, box_cls, box_delta, anchors, image_sizes):
		...
		for img_idx, anchors_per_image in enumerate(anchors):
			...
			box_cls_per_image = [box_cls_per_level[img_idx] for box_cls_per_level in box_cls]
			box_reg_per_image = [box_reg_per_level[img_idx] for box_reg_per_level in box_delta]
			results_per_image = self.inference_single_image_mod(
				box_cls_per_image, box_reg_per_image, anchors_per_image, tuple(image_size)
			)
			if results_per_image == -1:
				continue
	...
	def transform_deltas_to_bbox_per_image(self, box_cls, box_delta, anchors, gt_classes, image_size, threshold):
		...
		keep = batched_nms(boxes_all, scores_all, class_idxs_all, self.nms_threshold_kp)
		keep = keep[: self.max_kp_boxes_per_image]
		result = Instances(image_size)
